...
export function bridgeToolSearch({ body = {}, definition = {}, query, toolUseId } = {}) {
  ...
  if (history.continuation === "unsupported") {
    return { kind: "fallback", reason: "unsupported_tool_search_history" };
  }
  if (!TOOL_SEARCH_TYPES.has(definition.type)) {
    return {
      kind: "fallback",
      reason: isFutureServerToolType(definition.type)
        ? "unsupported_tool_search_version"
        : "unsupported_tool_search_type",
    };
  }
  ...
  const resultId =
    typeof toolUseId === "string" && toolUseId.length > 0
      ...
      : typeof definition.id === "string" && definition.id.length > 0
        ...
        : "tool_search";
  try {
    const toolReferences = searchDeferredTools({
      tools: Array.isArray(body.tools)
        ? body.tools.filter((tool) => tool?.defer_loading === true)
        : body.tools,
    ...
    return {
      kind: "result",
      block: createToolSearchResult({ toolUseId: resultId, toolReferences }),
    };
  } catch (error) {
    if (error?.code === "tool_search_fallback_required") {
      return { kind: "fallback", reason: error.reason ?? "unsupported_tool_search_request" };
    }
    return {
      kind: "result",
      block: mapToolSearchError({ toolUseId: resultId, error }),
    };
  }
}
...
export function searchDeferredTools({ tools = [], type, query, limit = 5 }) {
  ...
  if (tools.length > MAX_DEFERRED_TOOL_COUNT) {
    throw toolSearchFallback(
      "oversized_tool_catalog",
      "ToolSearch catalog exceeds the local search limit",
    );
  }
...
function appendSchemaSearchText(schema, append, hasCapacity) {
  ...
  while (pending.length > 0) {
    ...
    if (visited > MAX_SCHEMA_NODES) {
      throw toolSearchFallback(
        "oversized_tool_schema",
        "ToolSearch schema exceeds the local search limit",
      );
    }
  ...
  for (let index = 0; index < query.length; index += 1) {
    ...
    if (character === "\\") {
      ...
      if (unsupportedEscapes.has(escaped) || /[0-9]/.test(escaped)) {
        throw toolSearchFallback(
          "unsupported_python_regex",
          "Python regex feature requires provider ToolSearch",
        );
      }
    ...
    if (character === "^") {
      if (!atAlternativeStart) {
        throw toolSearchFallback(
          "unsupported_python_regex",
          "Python regex anchor requires provider ToolSearch",
        );
      }
      atAlternativeStart = false;
      continue;
...
function toolSearchFallback(reason, message) {
  return Object.assign(
    new CompatibilityProtocolError("tool_search_fallback_required", message),
    { reason },
  );
}
